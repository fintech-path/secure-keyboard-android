...
import android.keyboard.Keyboard.Key;
...
import com.hcxc.securekeyboard.R;
import com.hcxc.securekeyboard.SecureEncryptHelper;
import com.hcxc.securekeyboard.SecureKeyboardUtils;
...
public class KeyboardView extends View {
    ...
    private static final int REPEAT_START_DELAY = 100;
    ...
    private static final int MULTITAP_INTERVAL = 300; // milliseconds
    ...
    private SecureEncryptHelper helper;
    ...
    public void setEncryptHelper(SecureEncryptHelper helper) {
        this.helper = helper;
    }
    ...
    private void onBufferDraw() {
        Key[] tmp = mKeys.clone();
        List<Character> allLabels = SecureKeyboardUtils.INSTANCE.decryptKeyboard(mKeyboard, helper);
        int index = 0;
        for (Key key : mKeys) {
            if (key.codes[0] == Keyboard.KEYCODE_SHIFT || key.codes[0] == Keyboard.KEYCODE_DELETE || key.codes[0] == Keyboard.KEYCODE_SPACE || key.codes[0] == Keyboard.KEYCODE_POINT_NUMBER_BOARD) {
                continue;
            }
            key.label = String.valueOf(allLabels.get(index));
            key.codes[0] = (int) allLabels.get(index);
            index++;
        }
    ...
    private void detectAndSendKey(int index, int x, int y, long eventTime) {
        ...
        for (Key key : mKeys) {
            ...
            key.label = String.valueOf(allLabels.get(idx));
            key.codes[0] = (int) allLabels.get(idx);
            idx++;
        }
